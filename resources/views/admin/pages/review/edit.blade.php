            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                        <div class="row">
                            <div class="col-12">
                                <form action="{{ route('review.update',$customerReview->id) }}" method="post" id="mainForm"
                                        ...
                                        <div class="tab-pane fade  show active" id="underline-1" role="tabpanel"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input id="t-text" type="text" name="name"
                                                       class="form-control @error('name') is-invalid @enderror" value="{{$customerReview->name}}"
                                                       required>
                                                @error('name')
                                            ...
                                            <div class="tab-content" id="lineTabContent-3">
                                                @foreach($locales as $key => $locale)
                                                    <div class="tab-pane fade @if($key == 'tr')  show active @endif" id="underline-{{ $key }}" role="tabpanel" aria-labelledby="underline-{{ $key }}-tab">
                                                        <div class="form-group">
                                                            <p>Ünvanı ({{ $locale }})</p>
                                                            <input id="t-text" type="text" name="job[{{ $key }}]" class="form-control @error('job') is-invalid @enderror" value="{{$customerReview->getTranslation('job',$key)}}" required>
                                                            @error('job')
                                                            <div class="alert alert-danger">{{ $message }}</div>
                                                            @enderror
                                                        </div>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <textarea id="summernote" class="summernote {{$key}}" name="review[{{ $key }}] ">{{$customerReview->getTranslation('review',$key)}}</textarea>
                                                            @error('text')
                                                            <div class="alert alert-danger">{{ $message }}</div>
                                                            @enderror
                                                        </div>
