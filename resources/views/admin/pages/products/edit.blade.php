            ...
            <div class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area">
                        <form action="{{ route('products.update',$product->id) }}" method="post" id="mainForm"
                            ...
                            <div class="row">
                                        ...
                                        <div class="tab-content" id="lineTabContent-3">
                                            @foreach($locales as $key => $locale)
                                                <div class="tab-pane fade @if($key == 'tr')  show active @endif" id="underline-{{ $key }}" role="tabpanel" aria-labelledby="underline-{{ $key }}-tab">
                                                    <div class="form-group">
                                                        <p>Ürün Adı ({{ $locale }})</p>
                                                        <input id="t-text" type="text" name="title[{{ $key }}]" class="form-control @error('name') is-invalid @enderror" value="{{ $product->getTranslation('title',$key) }}" required>
                                                        @error('name')
                                                        <div class="alert alert-danger">{{ $message }}</div>
                                                        @enderror
                                                    </div>
                                                    <div class="form-group">
                                                        ...
                                                        <textarea class="summernote {{$key}}" name="text[{{ $key }}]" required>{{ $product->getTranslation('text',$key) }}</textarea>
                                                        @error('text')
                                                        <div class="alert alert-danger">{{ $message }}</div>
                                                        @enderror
                                                    </div>
                                                    <div class="form-group">
                                                        ...
                                                        <textarea class="summernote {{$key}}" name="properties[{{ $key }}]" >{{ $product->getTranslation('properties',$key) }}</textarea>
                                                        @error('text')
                                                        <div class="alert alert-danger">{{ $message }}</div>
                                                        @enderror
                                                    </div>
                                ...
                                <div class="tab-content col-6" id="lineTabContent-3">
                                    ...
                                    <div class="tab-pane fade  show active" id="underline-1" role="tabpanel"
                                         aria-labelledby="underline-tab">
                                        @if($product->image_tecnical!=null)
                                            <div class="form-group" style="margin-top: 20px">
                                                <table id="" class="table dt-table-hover" style="width:100%;padding: 15px">
                                                    <thead>
                                                    <tr>
                                                        <th>TEKNİK DETAY GÖRSELİ</th>
                                                    </tr>
                                                    </thead>
                                                    <tbody>
                                                    <tr>
                                                        <td>
                                                            <img width="500" height="300" src="/storage/{{ $product->image_tecnical }}">
                                                        </td>
                                                    </tr>
                                                    </tbody>
                                                    <tfoot>
                                                    <tr>
                                                        <th>(NOT: Eğer teknik detay görselini kaldırmak yerine güncellemek istiyorsanız sol taraftan yeni görseli eklemeniz yeterli olacaktır..)</th>
                                                    </tr>
                                                    </tfoot>
                                                </table>
                                            </div>
                                            <a href="{{route('admin.delete_image',$product->id)}}" class="btn btn-success btn-block mb-4 mr-2" style="margin-top: 20px">Teknik Detay Görselini Kalırmak İçin Tıklayınız</a>
                                        @endif
